...
public class GlobalClientSettingsMenu : MenuBase
{
    ...
    public override IEnumerable<IServerSpecificSetting> GetSettings(Player player)
    {
        ...
        {
            var id = UtilitiesPlugin.AutoGeneratedConfig.GlobalClientSettings.IndexOf(setting.Label);
            if (id < 0)
            {
                id = UtilitiesPlugin.AutoGeneratedConfig.GlobalClientSettings.Count;
                UtilitiesPlugin.AutoGeneratedConfig.GlobalClientSettings.Add(setting.Label);
                updated = true;
            }
